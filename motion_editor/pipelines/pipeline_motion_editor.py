...
@dataclass
class MotionEditorPipelineOutput(BaseOutput):
    images: Union[torch.Tensor, np.ndarray]


class MotionEditorPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 4. Prepare image
        controlnet_target_skeleton = torch.unsqueeze(skeleton[-1], dim=0)
        controlnet_batch_size = 1
        if isinstance(self.controlnet, ControlNetModel):
            images = self.prepare_image(
                image=controlnet_target_skeleton,
                width=width,
                height=height,
            ...
            print(images.size())  # [2, 8, 3, 512, 512]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                controlnet_latent_model_input = torch.cat([torch.unsqueeze(latent_model_input[1], dim=0), torch.unsqueeze(latent_model_input[3], dim=0)], dim=0)
                ...
                source_mid_block_res_sample = torch.zeros_like(mid_block_res_sample)
                mid_block_res_sample = torch.cat([torch.unsqueeze(source_mid_block_res_sample[0], dim=0), torch.unsqueeze(mid_block_res_sample[0], dim=0), torch.unsqueeze(source_mid_block_res_sample[1], dim=0), torch.unsqueeze(mid_block_res_sample[1], dim=0)], dim=0)
                ...
                noise_pred = self.unet(latent_model_input,
                                       ...
                                       target_masks=None,
                                       rectangle_source_masks=None,
                                       skeleton=None).sample.to(dtype=latents_dtype)
        ...
        return MotionEditorPipelineOutput(images=images)
