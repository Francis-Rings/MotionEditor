...
from motion_editor.models.temporal_attn import TemporalSelfAttention
...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, source_masks=None, target_masks=None, rectangle_source_masks=None, timestep=None, return_dict: bool = True, normal_infer: bool = False):
        # Input
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        attn_type = MotionFrameAttention if self.use_sc_attn else CrossAttention
        ...
        # Temp-Attn
        self.attn_temp = TemporalSelfAttention(
            query_dim=dim,
            heads=num_attention_heads,
            dim_head=attention_head_dim,
            dropout=dropout,
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None, normal_infer=False, source_masks=None, target_masks=None, rectangle_source_masks=None):
        ...
        if self.only_cross_attention:
            hidden_states = (
                self.attn1(norm_hidden_states, encoder_hidden_states, attention_mask=attention_mask, source_masks=source_masks, target_masks=target_masks, rectangle_source_masks=rectangle_source_masks) + hidden_states
            )
        else:
            if self.use_sc_attn or self.use_st_attn:
                ...
                hidden_states = self.attn1(norm_hidden_states, attention_mask=attention_mask, source_masks=source_masks, target_masks=target_masks, rectangle_source_masks=rectangle_source_masks) + hidden_states
        ...
        if self.attn2 is not None:
            ...
            hidden_states = (
                self.attn2(
                    norm_hidden_states, encoder_hidden_states=encoder_hidden_states, attention_mask=attention_mask, source_masks=source_masks, target_masks=target_masks, rectangle_source_masks=rectangle_source_masks
                )
                + hidden_states
            )
...
class MotionFrameAttention(CrossAttention):
    def forward_sc_attn(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, source_masks=None, target_masks=None, rectangle_source_masks=None):
        batch_size, sequence_length, _ = hidden_states.shape
        ...
        if inter_frame:
            ...
            former_frame_index[0] = 0
            key = rearrange(key, "(b f) d c -> b f d c", f=video_length)
            key = torch.cat([key[:, former_frame_index], key[:, curr_frame_index]], dim=2)
            key = rearrange(key, "b f d c -> (b f) d c")
            value = rearrange(value, "(b f) d c -> b f d c", f=video_length)
            value = torch.cat([value[:, former_frame_index], value[:, curr_frame_index]], dim=2)
            value = rearrange(value, "b f d c -> (b f) d c")
